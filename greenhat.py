...
def main(argv):
	...
	while i <= n:
		...
		for commit in range(0, num_commits):
			...
			#subprocess.call("git add realwork.txt; GIT_AUTHOR_DATE='" + curdate + "' GIT_COMMITTER_DATE='" + curdate , shell=True)
			
			subprocess.call("export GIT_COMMITTER_DATE="+curdate,shell=True)
			subprocess.call("export GIT_AUTHOR_DATE="+curdate,shell=True)
